...
<template>
    <v-app dark>
        <VAppBar>
            ...
            <VAppBarTitle
                :style="`height: 100%; align-items: center; max-width: ${show_nav_drawer_button ? '300px' : '80%'}`">
                <VAutocomplete @update:search="navigateTo(searchItemLink)" label="Suche" prepend-inner-icon="mdi-magnify"
                    variant="underlined" :items="items" item-text="title" v-model="searchItemLink" item-value="link"
        ...
        <VNavigationDrawer v-model="show_nav_drawer_button" class="navigation__drawer">
            <VList>
                <VListItem v-for=" item  in  items " :key="item.title" density="compact" :to="item.link"
