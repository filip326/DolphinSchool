        ...
        <VCardText>
            <div class="img">
                <section :key="index" v-for="_, index in  img ">
                    <div @click="updateColorPicker(index, i)" :style="{ backgroundColor: color }"
                        :class="pixelBorder ? 'pixelBorder' : ''" class="pixel" :key="i" v-for="color, i in img[index]">
                    </div>
                </section>
            </div>
        </VCardText>
        <VCardItem>
            <VColorPicker width="100%" swatches-max-height="200px" show-swatches :modes="['hex']" elevation="0" hide-sliders
                v-model="color" />
        </VCardItem>
        <VCardActions>
            <VBtn>Speichern</VBtn>
