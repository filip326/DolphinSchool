...
class BruteForceProtection {
    ...
    async isLoginAllowed(username: string, bypassToken?: string): Promise<MethodResult<boolean>> {
        if (!bypassToken) {
            ...
            try {
                const dbResult = await this.bruteForceProtection.findOne({ username: username, expires: { $gt: Date.now() } })
        ...
        try {
            const dbResult = this.bruteForceProtectionBypass.findOne({ username: username, token: bypassToken, expires: { $gt: Date.now() } })
            ...
            const dbResult2 = await this.bruteForceProtection.findOne({ username: username, token: bypassToken, expires: { $gt: Date.now() } })
    ...
    async reportFailedLoginAttempt(username: string, bypassToken?: string): Promise<MethodResult<boolean>> {
        ...
        if (bypassToken) {
            ...
            try {
                ...
                if (!dbResult2) {
                    try {
                        await this.bruteForceProtection.insertOne({ username: username, token: bypassToken, failedAttemts: 1, expires: Date.now() + 3 * 60 * 60 * 1000 })
                    } catch {
                        return [undefined, Error("Database error")]
                    }
                }
                // if there already is a BruteforceProtectionEntry, increment failed attempts and exceed expiration to 3 hours
                    ...
                    try {
                        await this.bruteForceProtection.updateOne({ username: username, token: bypassToken }, { $inc: { failedAttemts: 1 }, $set: { expires: Date.now() + 3 * 60 * 60 * 1000 } })
                    } catch {
                        return [undefined, Error("Database error")]
                    }
                }
        ...
        // if there is no valid BruteForceProtectionEntry, create one, valid for 3 hours
            ...
            try {
                await this.bruteForceProtection.insertOne({ username: username, failedAttemts: 1, expires: Date.now() + 3 * 60 * 60 * 1000 })
                return [true, null]
            } catch {
                return [undefined, Error("Database error")]
            }
    ...
    */
    async issueBypassToken(username: string): Promise<MethodResult<string>> {
        ...
        try {
            await this.bruteForceProtectionBypass.insertOne({ username: username, token: token, expires: Date.now() + 30 * 24 * 60 * 60 * 1000 })
            return [token, null]
        }
        catch {
            return [undefined, Error("Database error")]
    ...
    async exceedBypassToken(username: string, bypassToken: string): Promise<MethodResult<boolean>> {
        ...
        try {
            const dbResult = await this.bruteForceProtectionBypass.findOne({ username: username, token: bypassToken, expires: { $gt: Date.now() } })
            if (!dbResult) return [false, null]

            // if token exists, exceed expiration to 30 days
            const dbResult2 = await this.bruteForceProtectionBypass.updateOne({ username: username, token: bypassToken }, { $set: { expires: Date.now() + 30 * 24 * 60 * 60 * 1000 } })
