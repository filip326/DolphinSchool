...
class UserMessageManager implements IUserMessageManager {
    ...
    constructor(
        ...
        messageManager: IUserMessageManager
    ) {
    ...
    async getUserMessage(messageId: ObjectId): Promise<MethodResult<UserMessage>> {
        const dbResult = await this.userMessageCollection.findOne({
            _id: messageId
        });
        if (!dbResult) {
        ...
        return [
            new UserMessage(this.messageCollection, this.userMessageCollection, dbResult),
            null
        ];
    }
    ...
    async getMessages(filter: MessageFilterOptions): Promise<MethodResult<IUserMessage[]>> {
        ...
        const dbResult = await this.userMessageCollection
            .find(
                {
                    owner: this.userId,
    ...
    async deleteMessage(messageId: ObjectId, forAll: boolean): Promise<MethodResult<boolean>> {
        ...
        const dbResult2 = await this.userMessageCollection.findOne({
            message: messageId
        });
    ...
    async sendMessage(message: SendMessageOptions): Promise<MethodResult<IMessage>> {
        ...
        const dbResult2 = await this.userMessageCollection.insertMany(
        ...
        return [
            new Message(this.messageCollection, this.userMessageCollection, {
                _id: messageId,
                sender: this.userId,
                receivers: message.receivers,
                content: message.content,
